...
/**
 * The site-wide metadata every route inherits from. Inheritance itself is invisible here — it
 * does not exist until a route renders, and `tests/e2e/seo.spec.ts` asserts it per route over
 * HTTP. What this file guards is the shape of the source: the three Open Graph fields that must
 * stay absent, and the ones that must stay present.
 */
