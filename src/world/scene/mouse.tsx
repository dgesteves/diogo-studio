 ...
 *
 * Everything is built from one primitive — `createShell`, which skins a list of parametric
 * sheets into a single indexed geometry. Normals are computed per geometry, so a sheet is
 * smooth inside itself and every sheet boundary is a hard edge. Picking where the sheets
 * end is therefore how this file spells "crease", and it needs no seam meshes at all.
 *
...
/**
 * A measured curve: a table of (position, value) read off the photographs, sampled with a
 * monotone cubic. Monotone rather than plain Catmull-Rom because several of these tables
 * hold a deliberate step — the wheel slot opening, the nose flaring to full width in three
 * millimeters — and an interpolant that overshoots would put a bulge on either side of it.
 */
...
/**
 * A parametric sheet, and the only geometry primitive in this file. Everything — the palm
 * shell, each wall of each wing, the lit strip, the base plate — is a rectangular grid
 * evaluated through one of these, and the meshes below are just sheets grouped by material.
 */
export type Sheet = {
