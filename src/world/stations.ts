...
const RAW: Record<PageSlug, RawStation> = {
...
export const worldStations: Record<PageSlug, WorldStation> = Object.fromEntries(
  (Object.keys(RAW) as PageSlug[]).map((slug) => [slug, toStation(slug, RAW[slug])]),
) as Record<PageSlug, WorldStation>;

export function getStation(slug: PageSlug): WorldStation {
  return worldStations[slug];
}

export function useHoveredStation(): PageSlug | null {
  const state = useSyncExternalStore(subscribeWorld, getWorldSnapshot, getWorldServerSnapshot);
  return state.hovered;
}

export function useActiveStation(): PageSlug {
