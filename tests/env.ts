import type { env as appEnv } from "@/config/env";
...
// to `@/config/env` fails typecheck until this baseline accounts for it.
...
// `@/config/env` validates at import, so a spec cannot re-read `process.env` per case.
